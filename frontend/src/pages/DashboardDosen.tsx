...
import { getKelasForDosen, getDosenRatings } from '@/data/mockData';

const DashboardDosen = () => {
  const { user } = useAuth();
  const navigate = useNavigate();
  const kelasList = getKelasForDosen(user?.id || '');
  const ratings = getDosenRatings(user?.id || '');
  ...
  const spiderData = {
    labels: ['Leadership', 'Teamwork', 'Attitude', 'Communication', 'Innovation'],
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          ...
          <StatCard
            ...
            value={kelasList.reduce((acc, k) => acc + k.studentIds.length, 0)}
            icon={Users}
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          ...
          <motion.div
            ...
            className="bg-card rounded-xl p-6 shadow-soft border border-border"
          >
            <h3 className="text-xl font-bold text-foreground mb-4">Komentar Terbaru</h3>
            ...
            {kelasList.map((kelas, index) => (
              ...
              >
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  {kelas.studentIds.length} Mahasiswa
                </p>
              </motion.div>
            ))}
          </div>
        ...
        >
          ...
          <div className="flex gap-4">
            <Button onClick={() => navigate('/rating?type=mahasiswa')} className="flex-1">
              Rating Mahasiswa
            </Button>
            <Button onClick={() => navigate('/rating?type=dosen')} variant="outline" className="flex-1">
              Rating Dosen Lain
            </Button>
          </div>
        </motion.div>
